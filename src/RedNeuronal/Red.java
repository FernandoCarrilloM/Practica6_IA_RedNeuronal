...
class Red implements Serializable {
    ...
    /**
    * Crea una red de neuronas con el parametro que se pase. Se crean todas las neuronas y las
    sinapsis.
    ...
    * @param inputs Valores que se le pasan a las neuronas de entrada. Tiene que tener la misma
    *longitud que el num. de neuronas
    ...
    */
    public double[] epoca(ArrayList<Double> inputs) {
        ...
        //Recorrer la red para que cada Neurona vaya generando su valor y pasando a la
        //siguiente.
        ...
        for(int i=1; i<network.size();i++) {
            for(int j=0; j<network.get(i).length; j++) {
                ...
                //Actualiza su valor de salida en funcion de las entradas
            }
        }
        for(int i=0; i<network.get(network.size()-1).length;i++) {
            outs[i] = network.get(network.size()-1)[i].getResultado();
            //Actualiza el valor de salida de las neuronas de salida.
    ...
    */
    ...
    //Actualizar los pesos de las sinapsis (desde las neuronas de salida hasta la primera
    //capa oculta, ya que cada
